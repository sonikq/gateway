...
func (h *Handler) UpdateUserAccess(ctx *gin.Context) {
	...
	var request admin.UpdateUserAccessRequest
	...
	response := make(chan admin.UpdateUserAccessResponse, 1)

	c, cancel := context.WithTimeout(ctx, time.Second*time.Duration(h.config.CtxTimeout))
	defer cancel()

	go h.service.IUpdateUserMatrixByAdminService.UpdateUserMatrixByAdminUser(request, response)
