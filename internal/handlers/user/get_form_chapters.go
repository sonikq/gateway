...
func (h *Handler) GetFormChapters(ctx *gin.Context) {
	...
	request := user.GetFormChaptersRequest{
		UserID: userID,
		FormID: formID,
	}

	response := make(chan user.GetFormChaptersResponse, 1)

	c, cancel := context.WithTimeout(ctx, time.Second*time.Duration(h.config.CtxTimeout))
	defer cancel()

	go h.service.IUserService.GetFormChapters(request, response)
	defer func() {
		if r := recover(); r != nil {
