...
func (h *Handler) SaveChapterData(ctx *gin.Context) {
	...
	err = ctx.BindJSON(&dataJSON)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "invalid request json data"})
		h.log.Error("Invalid request json data", logger.Error(err))
		return
	}

	request := user.SaveChapterDataRequest{
		UserID:    userID,
		ChapterID: chapterID,
		JSON:      dataJSON,
	}

	response := make(chan user.SaveChapterDataResponse, 1)

	c, cancel := context.WithTimeout(ctx, time.Second*time.Duration(h.config.CtxTimeout))
	defer cancel()

	go h.service.IUserService.SaveChapterData(request, response)
	defer func() {
		if r := recover(); r != nil {
