...
func (h *Handler) CheckFormEmpty(ctx *gin.Context) {
	...
	request := user.CheckFormEmptyRequest{
		UserID: userID,
		FormID: formID,
	}

	response := make(chan user.CheckFormEmptyResponse, 1)

	c, cancel := context.WithTimeout(ctx, time.Second*time.Duration(h.config.CtxTimeout))
	defer cancel()

	go h.service.IUserService.CheckFormEmpty(request, response)
	defer func() {
		if r := recover(); r != nil {
