...
func (h *Handler) GetUIPermissions(ctx *gin.Context) {
	...
	request := user.GetUIPermissionsRequest{
		UserID: userID,
		FormID: formID,
	}

	response := make(chan user.GetUIPermissionsResponse, 1)

	c, cancel := context.WithTimeout(ctx, time.Second*time.Duration(h.config.CtxTimeout))
	defer cancel()

	go h.service.IUserService.GetUIPermissions(request, response)
	defer func() {
		if r := recover(); r != nil {
