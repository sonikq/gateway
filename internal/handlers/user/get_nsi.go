...
func (h *Handler) GetNSI(ctx *gin.Context) {
	...
	nsiCode := ctx.Query("nsi_code")
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{ErrMsgKey: "неверный тип nsi_code"})
		h.log.Error("Invalid request data", logger.Error(err))
		return
	}

	request := user.GetNSIRequest{
		UserID:  userID,
		NSICode: nsiCode,
	}

	response := make(chan user.GetNSIResponse, 1)

	c, cancel := context.WithTimeout(ctx, time.Second*time.Duration(h.config.CtxTimeout))
	defer cancel()

	go h.service.IUserService.GetNSI(request, response)
	defer func() {
		if r := recover(); r != nil {
