...
func (h *Handler) GetChapterData(ctx *gin.Context) {
	...
	request := user.GetChapterDataRequest{
		UserID:    userID,
		ChapterID: chapterID,
	}

	response := make(chan user.GetChapterDataResponse, 1)

	c, cancel := context.WithTimeout(ctx, time.Second*time.Duration(h.config.CtxTimeout))
	defer cancel()

	go h.service.IUserService.GetChapterData(request, response)
	defer func() {
		if r := recover(); r != nil {
