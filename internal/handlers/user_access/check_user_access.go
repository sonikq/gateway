...
func (h *Handler) CheckUserAccess(ctx *gin.Context) {
	...
	var request user_access.CheckUserAccessRequest
	...
	response := make(chan user_access.CheckUserAccessResponse, 1)

	c, cancel := context.WithTimeout(ctx, time.Second*time.Duration(h.config.CtxTimeout))
	defer cancel()

	go h.service.IUserAccessService.CheckAccessUser(request, response)
