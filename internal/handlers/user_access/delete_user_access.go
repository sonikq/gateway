...
func (h *Handler) DeleteUserAccess(ctx *gin.Context) {
	...
	var request user_access.DeleteUserAccessRequest
	...
	response := make(chan user_access.DeleteUserAccessResponse, 1)

	c, cancel := context.WithTimeout(ctx, time.Second*time.Duration(h.config.CtxTimeout))
	defer cancel()

	go h.service.IUserAccessService.DeleteAccessUser(request, response)
